...
- `VARIANTS` – regionale Varianten. `GUMASTER` – NPC-Gu-Meister mit ihren Gu (`gu`, der Meister nutzt das höchste Familienmitglied bis zu seinem Rang), Fraktion (`f`), `rang`, optional `stufe` und `hut`. `NPCTYPE` – Händler/NPC-Typen mit `trade` (`give`, `get`, optional `gu` und `gu_rang` für zufällige Gu eines Rangs) und optional `hut`. `hut` = Kopfbedeckung: `stroh`, `kappe`, `fell`, `band`, `kapuze`, `turban`, `krone`, `feder`.
