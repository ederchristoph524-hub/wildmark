...
func _test_demonic_ambush() -> void:
	...
	steps._check(Quests.current(&"bai_daemonen") == 1 and Quests.current(&"wu_wuerdig") == GameState.rank, "Aufgaben zählen besiegte Dämonen und den Rang")
	GameState.quests.erase(&"bai_daemonen")
	rogue.offer_choices()
