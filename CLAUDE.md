...
**Gebiete:** Die Welt besteht aus Gebieten (`docs/daten/gebiete.json` → `AreaData`, `BiomeData`); `GameState.area` ist das aktuelle. `World` baut daraus Gelände (`Terrain`: Rauschen + eingeebnete Plätze, Seen, Wege; Kacheln, Shader `assets/shaders/terrain.gdshader`), Vegetation nach Biom, Siedlungen (`Settlement` + `Architecture`, ein Mesh pro Dorf; Bewohner über `SettlementPeople`), Orte (`WorldAreas`: Materialquellen, `SpiritSpring`, `Inheritance`, Seen mit `WaterSurface`) und Hindernis-Orte. Reisen: Weltkarte → `EventBus.travel_requested` → `Main._on_travel`. Karten: `MapData`, `Minimap`, `MapMenu` (`AreaMapView`, `WorldMapView`). Grafik-Rundgang: `tools/capture_tour.gd` → `build/tour/`.
...
- Tests: `tests/test_formulas.gd`, `tests/test_data_import.gd`, `tests/test_data_registry.gd` (erwartete Anzahlen aus den Quelldateien über `ExpectedCounts`), der Durchspiel-Test `tests/test_game_smoke.gd` (Schritte in `game_smoke_steps.gd`, `combat_smoke_steps.gd`, `world_smoke_steps.gd`), `tests/test_childhood.gd` und der Katalog-Test `tests/test_gu_catalog.gd` (jeder Gu, Killer Move und jede Bestien-Fähigkeit; alle mit `--fixed-fps 60`), jeweils `godot --headless --path . --script res://tests/<name>.gd`.
