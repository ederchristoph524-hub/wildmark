...
extends Control
## Neues-Spiel-Menü: erster Gu, Talent, Todesmodus (Kindheit folgt später) oder Weiterspielen.
...
func _build(column: VBoxContainer) -> void:
	...
	column.add_child(UiTheme.label(tr("Die Südliche Grenze. Du bist gerade erwacht – deine Apertur ist offen, die Welt ist es nicht."), 18, UiTheme.MUTED))
	...
	column.add_child(UiTheme.label(tr("Kindheit: übersprungen (spielbare Kindheit folgt in einem späteren Stand)."), 16, UiTheme.MUTED))
	var start_text: String = tr("Erwachen – neues Spiel") + (tr(" (überschreibt den Spielstand)") if SaveSystem.has_save() else "")
